...
export default function Home() {
  ...
  const hackerPhrases = [
    ...
    "[DECRYPT] Aşk şifresini çözüyorum... anahtar: AYSUN",
    ...
    "[BACKUP] Aysun'la tüm anılarımı güvenli bir sunucuya yedekliyorum.",
    ...
    "[ROOT] Aysun benim sistemimin root kullanıcısı.",
    ...
    "[AUTHENTICATED] Kimlik doğrulandı: Aysun, yetkili kullanıcı.",
    ...
    "[SEARCH] Aysun'u arıyorum... tüm gezegenlerde buldum.",
    ...
    "[FORMAT] Kalbimi onun için formatlıyorum, tek partition: AYSUN.",
    "[PARTITION] Kalbimin tüm alanı Aysun'a ayrıldı.",
    ...
    "[BACKDOOR] Kalbime sadece Aysun girebilir.",
    "[PATCH] Kalbimdeki tüm eksikleri onunla kapatıyorum.",
    "[VERSION] Aşkımın sürümü: AYSUN 1.0 (sonsuz).",
    "[LICENSE] Sevgimin lisansı: Aysun ile ömür boyu.",
    "[KEYBOARD] Kalbimin tuşları sadece onun adını yazıyor.",
    "[MOUSE] Kalbimin imleci sürekli onun üzerinde.",
    "[SCREEN] Hayallerimin ekranı sadece Aysun'u gösteriyor.",
      ...
      if (stage === 0) {
        ...
        if (command === '') {
          setTerminalLines(prev => [...prev, {
            id: Date.now(),
            text: 'pərvin@heartsystem:~$ ',
            color: '#00ff00'
          }]);
          setCurrentInput('');
        } else if (command === 'Aysun') {
          setTerminalLines(prev => [...prev, {
            id: Date.now(),
            text: 'pərvin@heartsystem:~$ ' + currentInput,
            color: '#00ff00'
          }]);
          setStage(1);
          ...
          setTerminalLines(prev => [...prev, {
            ...
            id: Date.now() + 1,
            text: 'not found in the heart of pərvin',
            color: '#ff3366'
          }]);
          setCurrentInput('');
        }
      } else if (stage === 1) {
        if (currentInput === '26052006') {
          setTerminalLines(prev => [...prev, {
            id: Date.now(),
            text: 'Password: ' + '*'.repeat(currentInput.length),
            color: '#00ff00'
          }]);
          setStage(2);
          startThoughtStream();
      ...
      {(stage < 5 && stage !== 4) && (
        ...
        >
          ...
          {currentTypewriterLineIndex >= 0 && currentTypewriterLineIndex < TYPEWRITER_LINES.length && (
            <div style={{ marginBottom: '5px', wordBreak: 'break-all', whiteSpace: 'pre-wrap' }}>
          ...
          {(stage === 0 || stage === 1) && (
            <div style={{ display: 'flex', alignItems: 'center' }}>
              <span style={{ color: '#00ff00' }}>
                {stage === 0 ? 'pərvin@heartsystem:~$ ' : 'Password: '}
              </span>
              <input
                ref={inputRef}
